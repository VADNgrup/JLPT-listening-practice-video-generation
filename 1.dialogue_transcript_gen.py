...
LECTURE_BANK_PATH = Path(os.getenv("LECTURE_BANK_PATH", "ingredients/lecture_bank.json"))
SCRIPTS_OUTPUT_DIR = Path(os.getenv("SCRIPTS_OUTPUT_DIR", "outputs/jpc_scripts"))
...
4. You could use the following emotion tags naturally within the dialogue (compatible with the TTS model):
   {", ".join(TTS_TAGS)}
   But dont overuse them, only use when it fits the context and emotion of the dialogue line.
...
def main():
    import argparse
    parser = argparse.ArgumentParser(description="Generate dialogue scripts from grammar script JSON files.")
    parser.add_argument("-n", "--num-scripts", type=int, default=1, help="Number of scripts to generate per grammar point")
    ...
    for level in ["N5", "N4", "N3"]:
        ...
        for lec_id, info in grammar_lectures.items():
            print(f"Generating {args.num_scripts} scripts for Grammar: {info['lecture_name']} ({level})")
            ...
            while count < args.num_scripts:
                ...
                if conv:
                    ...
                    count += 1
                    print(f"  [{count}/{args.num_scripts}] Saved script to {script_filename}")
